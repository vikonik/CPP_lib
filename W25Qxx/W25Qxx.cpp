...
#include "cmsis_os.h"
#else
#define	W25qxx_Delay(delay)		pause->delay_ms(delay)
#endif
...
{
	uint8_t	ret;
    SSP_SendData(_W25QXX_SPI,Data);
    while(SSP_GetFlagStatus(_W25QXX_SPI, SSP_FLAG_BSY));
    ret = (SSP_ReceiveData(_W25QXX_SPI));
	return ret;	
}

void HAL_SPI_Receive(MDR_SSP_TypeDef* spi, uint8_t* pBuffer, uint16_t size, uint16_t a){
	while(size--){
    SSP_SendData(_W25QXX_SPI,*pBuffer);
    while(SSP_GetFlagStatus(_W25QXX_SPI, SSP_FLAG_BSY));
    *pBuffer = (SSP_ReceiveData(_W25QXX_SPI));
		pBuffer++;
	}
}

uint8_t testBuffWritePage[512];
void HAL_SPI_Transmit(MDR_SSP_TypeDef* spi,uint8_t* pBuffer,uint16_t size,uint16_t a){
	while(size--){
testBuffWritePage[size] = *pBuffer;

//W25qxx_Spi(*pBuffer++);
    SSP_SendData(_W25QXX_SPI,*pBuffer++);
    while(SSP_GetFlagStatus(_W25QXX_SPI, SSP_FLAG_BSY));
    SSP_ReceiveData(_W25QXX_SPI);//Костыль для миландра. Или надо с буфером разбираться. Задержка на 8 байт.
	}
}

void W25qxx_SPI_EnableSS_m(MDR_PORT_TypeDef* port,uint16_t pin){
	port->RXTX &= ~_W25QXX_CS_PIN;
}
...
uint32_t W25qxx_ReadID(void)
{
  ...
  W25qxx_SPI_EnableSS_m(_W25QXX_CS_GPIO,_W25QXX_CS_PIN);
  W25qxx_Spi(0x9F);
  Temp0 = W25qxx_Spi(W25QXX_DUMMY_BYTE);
  Temp1 = W25qxx_Spi(W25QXX_DUMMY_BYTE);
  Temp2 = W25qxx_Spi(W25QXX_DUMMY_BYTE);
  W25qxx_SPI_DisableSS_m(_W25QXX_CS_GPIO, _W25QXX_CS_PIN);
